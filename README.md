...
- **概览 KPI**：请求总数、会话数、错误率（4xx/5xx 或客户端断开 / 超时 / 中断）、平均（`x̅`）/ P95 / 最大耗时、上行与下行流量；上游响应带 usage 时追加 **总 Token**（输入 / 输出），配置定价表后追加**估算成本**。
...
- **Token 流量**：输入 / 输出 token 的逐时间桶双线图（仅在有 usage 数据时出现），左轴为 token 数，悬停显示每桶输入 / 输出。
